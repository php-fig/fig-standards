...
- После ключевых слов управляющих структур НЕОБХОДИМО ставить один пробел;
  после названий функций и методов — НЕДОПУСТИМО.
...
утилиты проверки стиля ДОЛЖНЫ предупреждать о нарушении мягкого ограничения,
...
> N.b.: Использование только пробелов без примеси табов помогает избежать
> проблем с диффами, патчами, историей и авторством строк. Использование пробелов
> также позволяет легко и точно сделать межстрочное выравнивание.
...
и открывающую фигурную, его НЕОБХОДИМО располагать на одной отдельной строке с
...
функции и открывающей круглой скобкой, а также НЕДОПУСТИМЫ пробелы после
...
- после открывающей круглой скобкой пробел НЕДОПУСТИМ;
...
на расположение пробелов, круглых и фигурных скобок, а также, что `else` и `elseif`
...
а также до и после ключевого слова `use`.
...
Ниже показаны примеры замыканий с аргументами и без, а также со списком
...
Обратите внимание, правила форматирования также применяются, когда замыкание
...
`indent_type`:
Тип отступов. `tab` — «Используется таб», `2` или `4` — «число пробелов»
